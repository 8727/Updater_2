...
namespace Updater_2
{
    internal class Shh_UpdateFactor
    {
        ...
        static bool EnsureRemoteDirectoryExists(string ipAddress, string remotePath)
        {
            try
            {
                ...
                {
                    ssh.Connect();
                    var mkdirCommand = ssh.RunCommand($"mkdir -p {directoryPath}");
                    if (mkdirCommand.ExitStatus != 0)
                    {
                        return false;
                    }
                    ssh.Disconnect();
        ...
        static async Task<bool> UploadFileViaSftp(string ipAddress, string localFilePath, string remotePath, int rowIndex, string fileName)
        {
            try
            {
                if (!EnsureRemoteDirectoryExists(ipAddress, remotePath))
                {
        ...
        static string GetState(string ipAddress)
        {
            try
            {
                using (var ssh = new SshClient(ipAddress, UI.ssh_port, UI.ssh_username, UI.ssh_password))
        ...
        static async Task<bool> ExecuteWebInstall(string ipAddress, string remoteTempPath, int rowIndex, string fileName)
        {
            ...
            //                    if (line != null && line.Contains("%"))
            //                    {
            //                        UI.StatusDataGridView(rowIndex, fileName, $"Web Uploading... {line.Trim()}", Color.Yellow);
            //                    }
            //                    await Task.Delay(100);
            //                }
            //            }
            ...
            //            UI.StatusDataGridView(rowIndex, fileName, "ERROR Web Uploading...", Color.Red);
            //            return false;
            //        }
            //    }
            //    await Task.Delay(DelayMs);
        ...
        static async Task<bool> CheckArchiveForDataJson(SshClient ssh, string remotePath)
        {
            try
            {
                var command = ssh.RunCommand($"tar -ztf {remotePath} | grep -x '\\./data.json'");
        ...
        static async Task<bool> ExtractArchive(SshClient ssh, string remotePath, string password)
        {
            try
            {
                var testCommand = ssh.RunCommand($"tar -tzf {remotePath} > /dev/null 2>&1; echo $?");
                if (testCommand.ExitStatus != 0 || testCommand.Result.Trim() != "0")
                {
                    return false;
                }

                var command = ssh.RunCommand($"echo '{password}' | sudo -S tar -xzf {remotePath} -C / 2>&1");
                return command.ExitStatus == 0;
            }
            catch
            {
        ...
        static async Task<bool> ProcessFileAsync(string ipAddress, string filePath, int rowIndex)
        {
            ...
            try
            {
                using (var ssh = new SshClient(ipAddress, UI.ssh_port, UI.ssh_username, UI.ssh_password))
                {
                    ...
                    if (extension == ".tar.gz")
                    {
                        UI.StatusDataGridView(rowIndex, fileName, "Archive or Installation package...", Color.Yellow);
                        if (await CheckArchiveForDataJson(ssh, remoteTempPath))
                        {
                            ...
                            await ExecuteWebInstall(ipAddress, remoteTempPath, rowIndex, fileName);
                            return false;    
                        }
                        else
                        {
